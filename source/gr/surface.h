...
bool gr_surface_alloc(gr_surface *s, const gr_pixelformat *pf, uint width, uint height, uint depth, uint mipmaps, uint layers, uint usage, uint flags);
...
static inline bool gr_surface_is_attachment(const gr_surface *s) { return (s->flags & GR_SURFACE_ATTACHMENT) != 0; }
static inline bool gr_surface_is_cube(const gr_surface *s) { return (s->flags & GR_SURFACE_CUBEMAP) != 0; }
static inline bool gr_surface_is_1d(  const gr_surface *s) { return !gr_surface_is_cube(s) && (s->height == 1) && (s->depth == 1); }
static inline bool gr_surface_is_2d(  const gr_surface *s) { return !gr_surface_is_cube(s) && (s->height != 1) && (s->depth == 1); }
static inline bool gr_surface_is_3d(  const gr_surface *s) { return !gr_surface_is_cube(s) && (s->depth  != 1); }
